...
module.exports = io => {
  ...
  nsp.on('connection', (socket) => {
    ...
    socket.on('JOIN_GAME', (name) => {
      ...
      const l = Object.keys(state.activeSockets).length
    ...
    socket.on('ACTION', (event) => {
      ...
      const receiver = {...state.enemies, ...state.activeSockets}[target.id];
      ...
      if(Object.values(state.takenAction).length == Object.values(state.activeSockets).length) {
    ...
    socket.on('disconnect', () => {
      console.log('disconnected ', socket.player);
  ...
  function applyEvents() {
    ...
    for(let i = 0; i < allEvents.length; i++) {
      ...
      const character = state.activeSockets[characterId].player;
