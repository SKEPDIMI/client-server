module.exports = {
  'attack-0': {
    ...
    damageFormula: (character, receiver) => {
      let criticalStrike = Math.random() + 0.9
      let damage = (character.entity.attack/receiver.entity.defense*criticalStrike).toFixed(2)

      let damageFormula = `${character.entity.attack} dmg. /${receiver.entity.defense} def. * ${criticalStrike.toFixed(2)} crit.`
  ...
  'attack-1': {
    ...
    damageFormula: (character, receiver) => {
      ...
      let criticalStrike = Math.random() + 0.85

      // need to take the enemy health and add 90% of their defense
      const damage = (base / defense * criticalStrike).toFixed(2);
